...
object Prompt {
  ...
  val startPointValidation: (Int, Int) => Boolean = (x: Int, y: Int) => x >= 1 && x <= gridNum && y == 1
  ...
  val endPointValidation: (Int, Int) => Boolean = (x: Int, y: Int) => x <= gridNum && x >= 1 && y == gridNum
  ...
  val towerLocationValidation: (Int, Int) => Boolean = (x: Int, y: Int) => x >= 1 && x <= gridNum && y >= 1 && y <= gridNum
    && (x, y) != Main.StartPt && (x, y) != Main.EndPt
}
